...
def train_model(df_feature, df_query):#df_feature 是召回数据添加特征后的数据，df_query 是线下验证数据
    #df_train = df_feature[df_feature['label'].notnull()]
    #df_test = df_feature[df_feature['label'].isnull()]
    train_idx = df_feature['label'].notnull()
    df_train = df_feature.loc[train_idx]

    test_idx = df_feature['label'].isnull()#真正的测试集
    df_test = df_feature.loc[test_idx]
    ...
    oof = []
    prediction = df_test[['user_id', 'article_id']].copy()#测试集的用户id和文章id
    prediction['pred'] = 0
    df_importance_list = []
    ...
    for fold_id, (trn_idx, val_idx) in enumerate(
        ...
        joblib.dump(lgb_model, f'./user_data2/model/lgb{fold_id}.pkl')
    ...
    df_oof.to_csv('./user_data2/prediction_result/oof_lgbm_valid.csv', index=False)
    prediction.to_csv('./user_data2/prediction_result/test_lgbm_valid.csv', index=False)
    ...
    os.makedirs('./user_data2/prediction_result', exist_ok=True)
    df_sub.to_csv(f'./user_data2/prediction_result/result_valid_lgbm.csv', index=False)
...
def online_predict(df_test):
    ...
    for fold_id in tqdm(range(5)):
        model = joblib.load(f'./user_data2/model/lgb{fold_id}.pkl')
        pred_test = model.predict_proba(df_test[feature_names])[:, 1]
        prediction['pred'] += pred_test / 5

    # 生成提交文件
    os.makedirs('./user_data2/prediction_result', exist_ok=True)
    prediction.to_csv('./user_data2/prediction_result/test_lgbm_online.csv', index=False)
    df_sub = gen_sub(prediction)
    df_sub.sort_values(['user_id'], inplace=True)
    df_sub.to_csv(f'./user_data2/prediction_result/result_online_lgbm.csv', index=False)
