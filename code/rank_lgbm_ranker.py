...
def train_model(df_feature, df_query):
    ...
    df_sub.to_csv('./user_data2/prediction_result/result_valid_lgbm_ranker_baseline.csv', index=False)
...
def online_predict(df_test):
    ...
    df_sub = gen_sub(prediction)
    df_sub.sort_values(['user_id'], inplace=True)
    df_sub.to_csv('./user_data2/prediction_result/result_online_lgbm_ranker_baseline.csv', index=False)


if __name__ == '__main__':
    if mode == 'valid':
        df_feature = pd.read_pickle('./user_data2/data/offline/feature_baseline.pkl')
        ...
        df_feature = pd.read_pickle('./user_data2/data/online/feature_baseline.pkl')
